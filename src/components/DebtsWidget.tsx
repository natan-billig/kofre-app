...
export const DebtsWidget: React.FC<DebtsWidgetProps> = ({
  ...
  return (
    <div className="rounded-3xl bg-slate-900 border border-slate-800 p-4 sm:p-5 space-y-4 shadow-sm">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {pendingDebts.length > 0 && (
            <span className="text-xs px-2 py-0.5 rounded-full bg-violet-500/15 text-violet-400 border border-violet-500/25 font-semibold">
              {pendingDebts.length}
            </span>
          )}
        </div>
        ...
        <button
          ...
          className="text-xs flex items-center gap-1.5 px-3 py-1.5 bg-violet-600 hover:bg-violet-500 active:scale-95 text-white font-medium rounded-xl shadow-sm shadow-violet-950/40 transition-all"
        >
      ...
      {/* Tabs */}
      <div className="flex items-center gap-1.5 bg-slate-950/50 p-1 rounded-xl border border-slate-800/80">
        <button
          type="button"
        ...
        <div className="text-center py-6 px-4 border border-dashed border-slate-800/70 rounded-2xl">
          ...
          {activeTab === 'pending' && (
            <button
              type="button"
              onClick={onOpenCreateDebt}
              className="mt-3 inline-flex items-center gap-1.5 px-3 py-1.5 bg-violet-600/20 hover:bg-violet-600/30 text-violet-400 border border-violet-500/30 text-xs font-medium rounded-xl transition-colors"
            >
              <Plus className="w-3.5 h-3.5" />
              <span>{t('debts.newDebt')}</span>
            </button>
                    ...
                    {isConfirmingDelete ? (
                      <div className="flex items-center gap-1 bg-slate-900 border border-slate-700 p-1 rounded-xl">
                        ...
                        <button
                          ...
                          className="px-1.5 py-0.5 text-slate-400 hover:text-white text-[10px] rounded-lg transition-colors"
                        >
                          ✕
                        </button>
                      </div>
      ...
      {settlingDebt && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="relative w-full max-w-sm bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <select
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-9 pr-4 py-2 text-xs text-white focus:outline-none focus:border-emerald-500/60"
                >
            ...
            <div className="flex items-center justify-end gap-2 pt-2">
              <button
                type="button"
                onClick={() => setSettlingDebt(null)}
                className="px-3 py-1.5 text-xs text-slate-400 hover:text-white rounded-xl transition-colors"
              >
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white text-xs font-semibold rounded-xl shadow-lg shadow-emerald-950/40 transition-all"
              >
