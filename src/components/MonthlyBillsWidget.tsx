...
export const MonthlyBillsWidget: React.FC<MonthlyBillsWidgetProps> = ({
  ...
  // Estado Vazio Elegante: Exibir card compacto com atalho de cadastro imediato
  if (filteredBills.length === 0) {
    return (
      <div className="rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 sm:p-5 space-y-3 shadow-sm transition-colors">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="p-1.5 text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white bg-slate-100 hover:bg-slate-200 dark:bg-slate-800/60 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700/60 rounded-xl transition-colors cursor-pointer"
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-950/40 border border-slate-200/70 dark:border-slate-800/70 text-center sm:text-left">
          <div>
            <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">
              {language.startsWith('es') ? 'Ningún gasto fijo registrado' : 'Nenhum gasto fixo cadastrado'}
            </p>
            <p className="text-[11px] text-slate-400 dark:text-slate-500">
              {language.startsWith('es')
                ? 'Organiza tus cuentas recurrentes con días de vencimiento'
                : 'Organize suas contas recorrentes com dias de vencimento'}
            </p>
          </div>
          <button
            type="button"
            onClick={onOpenManage}
            className="cursor-pointer px-3 py-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold flex items-center gap-1.5 shadow-sm shadow-indigo-600/30 transition-all active:scale-95 shrink-0"
          >
            <Plus className="w-3.5 h-3.5" />
            <span>{language.startsWith('es') ? 'Nuevo Gasto Fijo' : 'Novo Gasto Fixo'}</span>
          </button>
        </div>
      </div>
    )
